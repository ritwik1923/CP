...
int lowerlimit(int t,int n) {
    int low = 0, high = n;
    int ans = 0;
    while(low <= high) {
        int mid = low + (high-low)/2;
        int sum = 0;
        for(int i = 1;i <= mid && sum <= t;i++) {
            sum = sum + i*5;
        }

        //deb(sum) , deb(mid),deb2(low,high);

        if(sum <=  t)  /*star ,*/ ans = mid , low = mid + 1;
        else
            high = mid - 1;
...
int main() {
    ...
    //t=1;
    cin>>n>>t;
    int time_have = (12-8)*60 - t;
    //deb(time_have);
    cout<<lowerlimit(time_have,n);
}
